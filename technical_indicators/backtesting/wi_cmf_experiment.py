...
from ..indicators import price_sma, high_low, wi, cmf
...
def create_pos(script, date): # WI CMF 200MA
    ...
    try:
        ...
        if correct_date == None:
            return None
        if float(data["Sma"][script].loc[correct_date, "200 MA"]) <= float(data["Price"][script].loc[correct_date, "Close"]):
            ...
            if prev_date_index != 0:
                ...
                cur_cmf = float(data["Cmf"][script].loc[correct_date, "Indicator"])
                cur_wi = float(data["Wi"][script].loc[correct_date, "Indicator"])
...
def wi_cmf_experiment():
    ...
    with open("results/{0}/{1}_wi_cmf_15_200sma_sl_all.p".format(datetime.now().strftime("%Y-%m-%d"), datetime.now().strftime("%H:%M")), "wb") as f:
