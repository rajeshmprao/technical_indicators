...
def presentation_real():
    with open('results/2019-02-20/minervini_template_15sl_real.p', "rb") as f:
        data = pickle.load(f)
    
    date_list = data["date_list"]
    equity_list = [x/100 for x in data["tot_value_list"]]
    cash_list = data["cash_percent_list"]
    ...
    expectancy = winloss_ratio*avg_profit + (1-winloss_ratio)* avg_loss
    expectancy_days = winloss_ratio*avg_profit_days + (1-winloss_ratio)* avg_loss_days


    print("Results:")
    print("From Date = {0}\nTo Date = {1}\nTotal Positions = {2}\nProfit Positions = {3}\nLoss Positions = {4}\nWinLoss Ratio = {5}\nAverage Win = {6}\nAverage Loss = {7}\nAverage Win Days = {8}\nAverage Loss Days = {9}\nBeginning Equity = {10}\nFinal Equity = {11}\nMax Drawdown = {12}\nCurrent Drawdown = {13}\nCurrent Cash Percent = {14}\nExpectancy = {15}\n Expectancy Days = {16}\n".format(from_date, to_date, total_pos, profit_pos_number, loss_pos_number, winloss_ratio, avg_profit, avg_loss, avg_profit_days, avg_loss_days, beginning_value, cur_value, max_drawdown, cur_drawdown, cur_cash_pos, expectancy, expectancy_days))
